...
plot_related_code_trends_v2 <- function(Type, target_code, target_cui, codified_list, nlp_list, sample_labels, ONCE) {
  type_dict <- list("Diagnosis" = "PheCode", "Medication" = "RXNORM", "Lab" = "LOINC", "Procedure" = "CCS")
  ...
  plot_bar <- ggplot(bar_data, aes(x = description_label, y = Total_Patients, fill = description_label)) +
    geom_bar(aes(linetype = Sample), stat = "identity", position = position_dodge(width = 0.7), width = 0.6, color = "black") +
    fill_palette +
    scale_linetype_manual(values = c("solid", "dashed")[seq_along(unique(bar_data$Sample))]) +
