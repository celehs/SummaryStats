...
- Access the tool here: [ONCE Webapp](https://shiny.parse-health.org/ONCE/)
- Specify your search item (e.g., "Multiple Sclerosis")
- Download "Full Results" for NLP and codified features 
- Provide manual file paths to:
...
1. `paths`: A named list of file paths to raw data CSV files
    ...
    b. `paths$codified2`, `paths$nlp2` (required) – Sample 2
...
This module analyzes codified feature trends for a user-specified set of parent Phecodes, automatically including their child Phecodes (e.g., PheCode:250.1, PheCode:250.2) to assess hierarchical consistency. For each parent Phecode, it calculates annual prevalence rates and total patient counts, then generates visualizations to examine whether child features follow similar patterns across time and across datasets. These plots support quality control by highlighting potentially inconsistent or missing hierarchical relationships.We recommend starting with Phecodes 411 (Ischemic Heart Disease), 250 (Diabetes Mellitus), and 296 (Mood Disorders) — common conditions with well-established coding structures.
...
results_module3 <- module3(data_inputs, dictionary, sample_labels, phecodes = c("PheCode:411", "PheCode:250", "PheCode:296"))
