...
- **Module 2**: Target code prevalence
- **Module 3**: Trends for selected phenotypes
- **Module 4**: ONCE feature trends (e.g., diagnoses, medications, labs, procedures)
...
For each sample, you must provide a codified dataset containing the following columns:
...
- `feature_id` Code used in the codified dataset  
...
To run **Module 5**, you must provide a ONCE-derived feature similarity dictionary. This identifies the most relevant features (e.g., diagnoses, labs, medications) based on similarity to your selected target code. If you are not running this script on the O2 cluster, you will need to manually download these files from the ONCE webapp:

1. Access the tool here: [ONCE Webapp](https://shiny.parse-health.org/ONCE/)
2. Specify your search item (e.g., "Multiple Sclerosis")
3. Download "Full Results" for codified features 
4. Provide manual file path to the ONCE codified dictionary
...
2. Open the R Markdown file and define the following below:
   - File paths to the codified datasets
   - File path to your data dictionary
   - The **target Phecode** (`target_code`) you are analyzing
...
source("Helper_Functions_NoNLP_09302025.R")
...
This module loads and cleans the raw input datasets for downstream analysis. It processes codified clinical data, and imports a user-defined data dictionary. All datasets are standardized into a consistent format, ensuring compatibility with later modules. **Module 1 must be run first**, as it produces the foundational inputs required throughout the pipeline.

#### *Inputs*

1. `paths`: A named list of file paths to raw data CSV files
    a. `paths$codified1` (required) - Sample 1
    b. `paths$codified2` (optional) - Sample 2
2. `dictionary_path`: Path to a CSV file containing the raw data dictionary

#### *Outputs*

1. `data_inputs`: A list of cleaned codified datasets
...
2. `codified_plot`: A combined line-and-bar plot visualizing annual codified code trends
...
results_module2 <- plot_target_prevalence(data_inputs, target_code, sample_labels)
...
3. `sample_labels`: A named list labeling the samples (e.g., list("1" = "Site A", "2" = "Site B"))
4. `O2`: Logical flag indicating whether to load ONCE data from the shared O2 directory (TRUE) or from user-provided paths (FALSE)
5. `manual_ONCE_path_code`: File path to the codified ONCE dictionary (used if `O2` = FALSE)
6. `types`: A vector of clinical domains to evaluate (e.g., c("Diagnosis", "Medication", "Lab", "Procedure"))
7. `type_dict`: A named list mapping each domain to one or more feature prefixes
...
results_module5 <- plot_related_features(data_inputs, target_code, sample_labels, O2,
