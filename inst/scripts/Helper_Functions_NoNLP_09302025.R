...
clean_ONCE_data <- function(target_code, O2 = TRUE, path_code = NULL) {
  formatted_code <- gsub("[:.]", "_", target_code)
  
  if (O2) {
    path_code <- paste0("/n/data1/hsph/biostat/celehs/lab/sm731/ONCE_features/codified/", formatted_code, "_cod_features.csv")
  } else {
    if (is.null(path_code)) stop("Must provide manual codified ONCE path when O2 is FALSE.")
  }
...
plot_target_prevalence <- function(data_inputs, target_code, sample_labels) {
...
plot_related_code_trends_v2 <- function(Type, target_code, codified_list, sample_labels, ONCE,
...
plot_related_features <- function(data_inputs, target_code, sample_labels,
                                  O2 = FALSE,
                                  manual_ONCE_path_code = NULL,
                                  types = c("Diagnosis", "Medication", "Lab", "Procedure"),
                                  type_dict = list("Diagnosis" = "PheCode", 
                                                   "Medication" = "RXNORM", 
                                                   "Lab" = c("LOINC","ShortName","Other Lab"), 
                                                   "Procedure" = "CCS")) {
  ...
  ONCE <- clean_ONCE_data(target_code, O2, manual_ONCE_path_code)
